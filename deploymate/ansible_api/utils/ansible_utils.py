...
def setup_runner_environment(host, play_source, username=None, custom_roles_path=None):
    # ساخت مسیرهای موقت برای playbook
    ...
    with open(hosts_path, 'w') as hosts_file:
        if host:
            if isinstance(host, str):
                ...
                for h in host:
                    if username:
                        hosts_file.write(f'{h} ansible_user={username}\n')
                    else:
                        hosts_file.write(h + '\n')
        else:
            hosts_file.write('[all]\nlocalhost ansible_connection=local')
...
def install_tool(host, role_name, username=None, custom_roles_path=None, use_local_connection=False, password=None):
    # تعریف playbook برای نصب ابزار
    play_source = f"""
---
- name: Install and configure {role_name}
  ...
  roles:
    ...
    base_path, playbook_name, inventory_path = setup_runner_environment(host, play_source, username, custom_roles_path)
